...
define([
	...
	'lsp/utils'
], function(objects, messages, Deferred, lib, mUIUtils, mExplorer, mCommands, URITemplate, EventTarget, i18nUtil, KeyBinding, mGlobalCommands, mSlideout, mLanguageServerRegistry, Utils) {
	...
	objects.mixin(Outliner.prototype, /** @lends orion.outliner.Outliner.prototype */ {
		...
		setContentType: function(fileContentType, title) {
			...
			if (lspServer) {
				...
				var allOutlineProviders = this._serviceRegistry.getServiceReferences("orion.edit.outliner");
				...
				filteredProviders = this.filteredProviders = allOutlineProviders.filter(function(serviceReference) {
					...
					if (contentTypeIds) {
						return contentTypeIds.some(function(contentTypeId) {
							return this._contentTypeRegistry.isExtensionOf(fileContentType, contentTypeId);
						}.bind(this));
				...
				this._providerLookup = false;
			}

			this._outlineService.setOutlineProviders(filteredProviders);
			this.setOutlineProviders(filteredProviders);
			this.generateOutline();
